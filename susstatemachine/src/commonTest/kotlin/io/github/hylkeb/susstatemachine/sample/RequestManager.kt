...
import io.github.hylkeb.susstatemachine.StateMachine.Companion.awaitNextState
...
class RequestManager internal constructor(
    ...
    public constructor(
        ...
        coroutineScope: CoroutineScope,
    ) : this(RealDependencyContainer(api, requestStateObserver), coroutineScope)

    val stateMachine: StateMachine<RequestState> by lazy { dependencyContainer.requestStateMachine }

    init {
        coroutineScope.launch { stateMachine.run() }
    }

    suspend fun getResponse(): Result<String> {
        val currentState = stateMachine.stateFlow.value
        when (currentState) {
        ...
        stateMachine.awaitNextState(currentState)
